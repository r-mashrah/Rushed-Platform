...
try {
    ...
    // Fallback if homedir is somehow missing or weird in environment
    const desktopPath = path.join(userProfile, 'Desktop', 'students_import_template.xlsx');

    XLSX.writeFile(wb, desktopPath);
    console.log(`FILE_CREATED:${desktopPath}`);
} catch (error) {
